...
exposes a built-in control UI for switching active scenarios at runtime.`,
  ...
  gigamock --logger-level INFO --logger-pretty-print`,
	...
	rootCmd.Flags().StringVar(&cfg.GRPCTLSCertFile, "grpc-tls-cert-file", cfg.GRPCTLSCertFile, "gRPC TLS server certificate file")
	rootCmd.Flags().StringVar(&cfg.GRPCTLSKeyFile, "grpc-tls-key-file", cfg.GRPCTLSKeyFile, "gRPC TLS server private key file")
	rootCmd.Flags().StringVar(&cfg.GRPCTLSCAFile, "grpc-tls-client-ca-file", cfg.GRPCTLSCAFile, "gRPC client CA file for mTLS")
	rootCmd.Flags().IntVar(&cfg.GRPCStreamMax, "grpc-stream-max-messages", cfg.GRPCStreamMax, "maximum messages accepted per gRPC stream")
	rootCmd.Flags().IntVar(&cfg.GRPCStreamTimeout, "grpc-stream-timeout-seconds", cfg.GRPCStreamTimeout, "gRPC stream timeout in seconds")
